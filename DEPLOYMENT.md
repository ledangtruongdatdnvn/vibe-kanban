...
- **admin**: Temporary admin UI for credentials, cleanup, and branch/workspace maintenance (port 3005)
...
admin:     (assign only when you need the admin console)
```

> `host` should remain internal-only. Expose `admin` only when you need it, and protect it with `ADMIN_SECRET`.
...
```bash
docker ps | grep -E "host|relay|remote|admin|electric"
```
...
4. If `loggedIn: false` → re-paste credentials via admin (Step 5).
...
### "EACCES: permission denied" when saving Codex credentials in admin
...
```bash
docker exec -u root $(docker ps --filter "name=admin" --format "{{.Names}}") chmod -R 777 /creds
```
...
Paste output into the "Codex" field in admin (`http://localhost:3005`).
